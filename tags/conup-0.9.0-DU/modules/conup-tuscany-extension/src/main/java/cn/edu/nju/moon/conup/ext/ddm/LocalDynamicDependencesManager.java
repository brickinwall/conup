...
public class LocalDynamicDependencesManager {
	...
	/**	
	 * When a sub-transaction begins, you can get whether the component will be used in the future
	 * @param reference
	 *           Service with fully qualified name	     	
	 */
		...
		if(getTempFuture().contains(referenceTemp)){			
			...
			for (String e : eveinf) {
				if (e.contains(referenceTemp)) {
					...
					if(states.get(tempNextState).getFuture().contains(referenceTemp)){
						LOGGER.fine(reference + "is in use now and will be used in transaction "+transactionID+" in future!");
						return true;
					}
					else{
						LOGGER.fine(reference + " is in use now, but will not use in transaction "+transactionID+" in future!");
						return false;
					}}
			}
			LOGGER.fine(reference + " is not in use now, but will be used in the future!");			
			return true;
		}
		else{
