...
public class LaunchPortal {
	private static Logger LOGGER = Logger.getLogger(LaunchPortal.class
			.getName());

	public static void main(String[] args) throws Exception {
		LOGGER.fine("Starting conup-sample-portal node ....");
		String contributionURL = ContributionLocationHelper
				.getContributionLocation(LaunchPortal.class);
		...
		CompLifecycleManagerImpl compLifecycleManager = new CompLifecycleManagerImpl(
				compObj);
		...
		DynamicDepManager depMgr = NodeManager.getInstance()
				.getDynamicDepManager(compObj.getIdentifier());
		...
		OndemandSetupHelper ondemandHelper = nodeMgr
				.getOndemandSetupHelper(compObj.getIdentifier());
		UpdateManager updateMgr = nodeMgr.getUpdateManageer(compIdentifier);

		CommServerManager.getInstance().start(compIdentifier);

		ServerIoHandler serverIoHandler = CommServerManager.getInstance()
				.getCommServer(compIdentifier).getServerIOHandler();
	...
	public static void accessServices(Node node) throws Exception {
		...
		while (scanner.hasNextLine()) {
			...
			try {
				...
				System.out
						.println("Unsupported command. input 'help' for help.");
				continue;
			}
			...
			case access:
				...
				// System.out.println("accessTimes: " + rqstInterval + " " +
				// accessTimes);
			...
			case update:
				...
				if (input.length == 3) {
					...
					AuthCompUpdate.update(targetComp, toVer, false);
				} else {
...
//				if (input.length <= 4 || input.length % 2 == 1) {
				...
				for (int i = 4; i < input.length && i + 1 < input.length; i += 2) {
				...
				for (int i = 0; i < accessTimes; i++) {
					...
					if (updatePoints.get(i) != null) {
						// System.out.println("update " + targetComp + " at " +
						// i);
						AuthCompUpdate.update(targetComp, updatePoints.get(i), scopeFlag);
					}
				}
				break;
			case ondemand:
				if (input.length == 2) {
					targetComp = input[1].trim();
					AuthCompUpdate.ondemand(targetComp);
				} else {
					System.out.println("Illegal parameters for 'update'");
					break;
				}
	...
	private static void printHelp() {
		...
		System.out.println("	[usage] updateAt AuthComponent 100 50 35 VER_ONE [SCOPE]");
		...
		System.out.println("	[usage] updateAt AuthComponent 100 100 25 VER_ONE 65 VER_TWO [SCOPE]");
		System.out.println("	[behavior] access 50 times, and the thread sleep 200ms before sending each request. "
