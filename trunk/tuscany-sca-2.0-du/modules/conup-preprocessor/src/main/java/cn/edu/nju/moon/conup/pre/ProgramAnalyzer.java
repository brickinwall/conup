...
public class ProgramAnalyzer {
	 ...
	 * @param targetPath
	 */
	public static void analyzeApplication(String sourcePath,String tempPath,String targetPath){
		...
		if(fileName.endsWith(".jar")||fileName.endsWith(".war")||fileName.endsWith(".ear")||fileName.endsWith(".zip")){
			...
			analyse.analyzeJar(sourcePath, tempLocation, outputLocation);
		}
		else
		{
			analyse.analyzeSource(sourcePath);
		}
	}
	/**
	 ...
	 * @param tempPath
	 */
	public static void analyzeApplication(String sourcePath,String tempPath){
		ProgramAnalyzer analyse = new ProgramAnalyzer();		
		String[] names = sourcePath.split("/");
		String fileName = names[names.length-1];
		if(fileName.endsWith(".jar")||fileName.endsWith(".war")||fileName.endsWith(".ear")||fileName.endsWith(".zip")){
			String tempLocation = tempPath+fileName.substring(0, fileName.length()-4);			
			analyse.analyzeJar(sourcePath, tempLocation, sourcePath);
		}
		else
		{
			analyse.analyzeSource(sourcePath);
...
//			ProgramAnalyzer.analyzeApplication("/home/analyzed/conup-sample-portal.jar", "/home/temp/", "/home/analyzed/");
			ProgramAnalyzer.analyzeApplication("/home/analyzed/conup-sample-auth.jar", "/home/temp/");
